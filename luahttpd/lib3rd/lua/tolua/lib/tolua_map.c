...
TOLUA_API int tolua_dobuffer(lua_State* L, char* B, unsigned int size, const char* name) {

 #ifdef LUA_VERSION_NUM /* lua 5.1 */
 return luaL_loadbuffer(L, B, size, name) || lua_pcall(L, 0, 0, 0);
 #else
 return lua_dobuffer(L, B, size, name);
 #endif
};
